  ...
  for row in rows:
    try:
      ...
      with open(fname, 'rb') as fixed_dump_file:
        ...
        hbc.put_fixed_dump(ooid, fixed_dump, add_to_unprocessed_queue = True, submitted_timestamp = datetime.utcnow())
...
def get_last_run_date(config):
  try:
    ...
    return datetime.utcnow() - timedelta(days=config.daysIntoPast)
